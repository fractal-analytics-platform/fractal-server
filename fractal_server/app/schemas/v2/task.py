...
class TaskImportV2(BaseModel, extra=Extra.forbid):
    ...
    version: Optional[str] = None
    name: Optional[str] = None
    _source = validator("source", allow_reuse=True)(
        valstr("source", accept_none=True)
    )
    _pkg_name = validator("pkg_name", allow_reuse=True)(
        valstr("pkg_name", accept_none=True)
    )
    _version = validator("version", allow_reuse=True)(
        valstr("version", accept_none=True)
    )
    _name = validator("name", allow_reuse=True)(
        valstr("name", accept_none=True)
    )
