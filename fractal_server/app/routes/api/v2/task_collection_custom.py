...
async def collect_task_custom(
    ...
    if overlapping_sources_v2:
        overlapping_tasks_v2_source_and_id = [
            f"Source: {task.source}\tTaskV2 ID: {task.id}\n"
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                f"{''.join(overlapping_tasks_v2_source_and_id)}"
            ),
        )
    ...
    if overlapping_sources_v1:
        overlapping_tasks_v1_source_and_id = [
            f"Source: {task.source}\tTaskV1 ID: {task.id}\n"
            for task in overlapping_sources_v2
        ]
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                ...
                f"{''.join(overlapping_tasks_v1_source_and_id)}"
            ),
        )
