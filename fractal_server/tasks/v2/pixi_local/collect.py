...
def collect_local_pixi(
    *,
    ...
    task_group_id: int,
    targz_file: WheelFile | None = None,
) -> None:
    """
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        with next(get_sync_db()) as db:
            ...
            try:
                ...
                # Write .tar.gz file and set task_group.wheel_path
                if targz_file is not None:

                    targz_path = (
                        Path(task_group.path) / targz_file.filename
                    ).as_posix()
                    logger.info(f"Write targz-file contents into {targz_path}")
                    with open(targz_path, "wb") as f:
                        f.write(targz_file.contents)
                    task_group.wheel_path = targz_path
                    task_group = add_commit_refresh(obj=task_group, db=db)

                # Prepare replacements for templates
                replacements = [
                    ("__PACKAGE_ENV_DIR__", task_group.venv_path),
                    ("__TAR_GZ_PATH__", targz_path),
                ...
                _customize_and_run_template(
                    template_filename="pixi_template.sh",
            ...
            #         # Check package_name match between pip show and taskgroup
            #         task_group = db.get(TaskGroupV2, task_group_id)
            ...
            #         package_root_parent = pkg_attrs.pop(
            #           "package_root_parent")
            ...
            #         )
            #         logger.info(
            #           "create_db_tasks_and_update_task_group - end"
            #         )
