...
# Install pinned packages (note: do not quote $PINNED_PACKAGE_LIST_POST since it could be e.g. "numpy==1.2.3 torch=3.2.1")
if [ "$PINNED_PACKAGE_LIST_POST" != "" ]; then
    ...
    write_log "SKIP installing pinned versions $PINNED_PACKAGE_LIST_POST (empty list)"
fi
