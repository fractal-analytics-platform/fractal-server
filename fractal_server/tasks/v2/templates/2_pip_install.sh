...
if [ "$PINNED_PACKAGE_LIST_PRE" != "" ]; then
    ...
    "$VENVPYTHON" -m pip install ${FRACTAL_PIP_CACHE_DIR_zzz_ARG} $PINNED_PACKAGE_LIST_PRE
...
"$VENVPYTHON" -m pip install ${FRACTAL_PIP_CACHE_DIR_zzz_ARG} "$INSTALL_STRING"
...
if [ "$PINNED_PACKAGE_LIST_POST" != "" ]; then
    ...
    "$VENVPYTHON" -m pip install ${FRACTAL_PIP_CACHE_DIR_zzz_ARG} $PINNED_PACKAGE_LIST_POST
