...
Fractal Server provides an authentication backend (Bearer), based on the [JWT](https://fastapi-users.github.io/fastapi-users/latest/configuration/authentication/strategies/jwt/) strategy, which produces the `/auth/token/login/` route.
