...
def test__get_final_set(tmp_path: Path):
    ...
    with pytest.raises(ValidationError):
        ...
        _get_final_set(public_tasks_path=path1)

    path1.write_text(json.dumps([["a", "b", "c"], ["a", "b", "c"]]))
    assert _get_final_set(public_tasks_path=path1) == {
        ("a", "b", "c"),
    }
    ...
    final_list = _get_final_set(
        public_tasks_path=path1,
        instance_tasks_path=path2,
        instance_ignore_path=path3,
    )
    print(final_list)
    assert final_list == {
        ("pkg2", "1.2.3", "A"),
