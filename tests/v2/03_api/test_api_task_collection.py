...
async def test_task_collection_from_wheel_non_canonical(
    ...
    """
    Same as test_task_collection_from_wheel, but package has a non-canonical name.
    """
    ...
    wheel_path = (
        testdata_path.parent
        / "v2/fractal_tasks_mock_capital/dist"
        / "FrAcTaL_TaSkS_MoCk_NoN_CaNoNiCaL-0.0.1-py3-none-any.whl"
    )
    payload_package = wheel_path.as_posix()
    ...
    expected_python_version = CURRENT_PYTHON
    debug(payload)
    ...
    async with MockCurrentUser(user_kwargs=dict(is_verified=True)):
        ...
        state_id = state["id"]
        venv_path = state["data"]["venv_path"]
        assert "fractal-tasks-mock-non-canonical" in venv_path
