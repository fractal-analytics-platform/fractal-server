...
async def test_loss_of_access_to_task(
    ...
    user_A = UserOAuth(email="a@a.a", **attrs)
    user_B = UserOAuth(email="b@b.b", **attrs)
    user_A.settings = UserSettings()
    user_B.settings = UserSettings()
    team_group = UserGroup(name="team")
    db.add(user_A)
    db.add(user_B)
    db.add(team_group)
    await db.commit()
    ...
    task_A = await task_factory_v2(
        command_non_parallel="echo", source="1", user_id=user_A.id
    )
    ...
    async with MockCurrentUser(user_kwargs=dict(id=user_A.id)) as user:
        ...
        job_id = res.json()["id"]
        res = await client.get(f"/api/v2/project/{project.id}/job/{job_id}/")
        assert res.status_code == 200
    ...
    async with MockCurrentUser(user_kwargs=dict(id=user_A.id)) as user:
        ...
        assert res.status_code == 403
        assert "Current user has no read access" in str(res.json()["detail"])
        ...
        assert (
            wftasks[1]["warning"] == "Current user has no access to this task."
        )
