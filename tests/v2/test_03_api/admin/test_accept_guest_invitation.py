...
async def test_project_sharing(
    ...
    async with MockCurrentUser(profile_id=profile.id):
        # Create projects
        res = await client.post("/api/v2/project/", json=dict(name="pname"))
        assert res.status_code == 201
        project_id = res.json()["id"]
        # Invite guest and standard users
        res = await client.post(
            f"/api/v2/project/{project_id}/guest/?email={guest_user.email}",
            json=dict(permissions=ProjectPermissions.READ),
        )
        assert res.status_code == 201
        res = await client.post(
            f"/api/v2/project/{project_id}/guest/?email={standard_user.email}",
            json=dict(permissions=ProjectPermissions.READ),
        )
        assert res.status_code == 201
    ...
    async with MockCurrentUser(is_superuser=True):
        # Accept invitation for guest user
        res = await client.post(
            (
                f"/admin/v2/linkuserproject/verify/"
                f"?project_id={project_id}&guest_user_id={guest_user.id}"
            )
        )
        ...
        res = await client.post(
            (
                f"/admin/v2/linkuserproject/verify/"
                f"?project_id={project_id}&guest_user_id={standard_user.id}"
            )
        )
